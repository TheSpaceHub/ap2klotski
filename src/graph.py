...
def load_puzzle(filepath):
    """Llegeix el fitxer JSON del puzzle."""
...
def generate_graph(puzzle):
    ...
    start_state = tuple(tuple(pos) for pos in puzzle['start'])
    ...
    vp_state = g.new_vertex_property("string") # Guardarem l'estat com a JSON string per recuperar-lo
    ...
    print(f"Generant graf... Aquesta operació pot trigar segons la complexitat del puzzle.")
    ...
    print(f"Graf completat: {g.num_vertices()} estats (vèrtexs) i {g.num_edges()} moviments (arestes).")
    return g
...
def main():
    ...
    out_filepath = filepath.replace(".json", ".graphml")
